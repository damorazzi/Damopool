...
// Third verbatim copy of this function (overview.js, pool.js, now
// here). pool.js's own duplication comment named "once a third page
// needs the same logic" as the moment to revisit extracting it into a
// shared module -- that moment has now arrived. Still not extracted
// here: DEVELOPMENT_PROCESS.md Section 5 treats editing a previously-
// shipped file (overview.js or pool.js, both already committed and
// pushed) as Permanent Human Governance absent a regression, so the
// actual extraction remains a separate, explicitly human-approved
// step, not something this milestone decides unilaterally -- but the
// trigger condition itself is worth recording plainly rather than
// repeating the copy a third time with no acknowledgment.
