...
`schema_version` `"1.4"` (Phase E Milestone 30 bumped this from `"1.3"`
— additive, backward-compatible: a new `block_progress` field on
`pool`/`users[...]`/`workers[...]`, nothing removed or reshaped):

```
{
  "metadata": {
    "schema_version": "1.4", "generated_at": <ISO8601>, "generator": <str>,
