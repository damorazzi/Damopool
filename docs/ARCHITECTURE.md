...
`schema_version` `"1.5"` (Phase E Milestone 31 bumped this from `"1.4"`
— additive, backward-compatible: three new `session_accepted_count`/
`session_rejected_count`/`session_started_at` fields on `workers[...]`
only, nothing removed or reshaped):

```
{
  "metadata": {
    "schema_version": "1.5", "generated_at": <ISO8601>, "generator": <str>,
  ...
  "pool": {
    ...
    "block_progress": {
      "best_share_difficulty", "network_difficulty": <float|null>,
      "progress_percent", "still_needed_multiplier": <float|null>
    }
  },
  "users": {
    "<username>": {
      ...same scope fields as pool (including hashrate_1m/hashrate_24h,
      difficulty_histogram, block_progress; no top-level
      network_difficulty -- pool-only, though block_progress.
      network_difficulty carries the same pool-wide value)...,
      "workers": [<workername>, ...],
      "rolling_windows": {...}
    }
  },
  "workers": {
    "<workername>": {
      ...
      network_difficulty carries the same pool-wide value)...,
      "rolling_windows": {...},
      "session_accepted_count", "session_rejected_count": <int>,
...
`block_progress` (Phase E Milestone 30: Block Progress Analytics) is a
purely educational, purely descriptive field — an explicit Human
requirement rules out any predictive or probabilistic framing ("not
intended to predict future block finds or estimate probability"). It
compares one scope's own best solved share against the current network
difficulty, expressed two ways: `progress_percent` (what fraction of
today's network difficulty the best share has reached) and
`still_needed_multiplier` (how many times larger the best share would
need to be to match it). Generated by a new, dedicated, stateless pure
function, `block_progress.compute_block_progress()` — unlike
`histogram_builder.py`/`ckpool_native_stats.py`, this needs no new data
source and no incremental state of its own: `best_share_difficulty` is
each scope's own already-computed `best_share_ever.sdiff`, and
`network_difficulty` is the same pool-wide value `ckpool_native_stats.
read_network_difficulty()` already produces (Milestone 29) — this field
exists on `pool`/`users[...]`/`workers[...]` individually (unlike the
top-level `network_difficulty` field, which is pool-only), so every
scope carries its own copy of the same pool-wide value alongside its own
best share, and no frontend cross-reference into `payload.pool` is
needed to render it. `progress_percent`/`still_needed_multiplier` are
`null` whenever either input is missing, non-finite, or not strictly
positive — this is computed with an explicit divide-by-zero guard and
never produces a NaN/Infinity value (which `json.dump` would otherwise
serialize as a non-standard, `JSON.parse`-rejecting token). A user/
worker with no solved share at all still gets a well-formed (all keys
present, ratios `null`) `block_progress`, never a missing key.

`session_accepted_count`/`session_rejected_count`/`session_started_at`
(Phase E Milestone 31: Worker Session Accepted/Rejected Counts) are
worker-scope-only (no pool/user equivalent — a "current connection
session" is not a coherent concept at either of those scopes) counts
of accepted/rejected shares submitted by the worker's CURRENT
connection, alongside (never replacing) the existing lifetime
`accepted_count`/`rejected_count`. Generated by a new, dedicated,
independent module, `worker_sessions.py`, on the same architectural
precedent as `histogram_builder.py`/`ckpool_native_stats.py`/
`block_progress.py`. A "session" boundary is detected via the
share's own `enonce1` field (the stratum extranonce1), not `clientid`
— investigation (2026-07-23) found `clientid` is a small, explicitly
recycled connector-slot integer (confirmed reused across entirely
different users in this pool's own real history), while `enonce1`
never repeated across any of the 42 distinct connections in this
pool's full observed history, consistent with its actual protocol
purpose (partitioning each connection's own nonce search space to
prevent hash collisions between concurrently-mining clients — a
CKPool protocol bug to violate, not merely an inconvenience for this
feature). `clientid` is still recorded internally for display/
debugging but is never the disambiguating key.

Disclosed, Human-acknowledged limitation: there is no proof `enonce1`
can never repeat over an arbitrarily long pool lifetime, only that it
is protocol-designed for uniqueness and has never repeated once in
this pool's history. No `ckpool.log` dependency and no inactivity
timeout exist to mitigate this (both explicitly ruled out by Human
instruction) — a session is reset only when a worker's shares carry a
genuinely new `enonce1`, never on elapsed quiet time, and a connection
resuming after any length of silence under its own already-known
`enonce1` correctly continues its existing session rather than
resetting.

Unlike `histogram_builder.py`'s/`analytics_state.py`'s own forever-
cumulative, order-*independent* per-file accumulation (safe to merge
in any order), session detection is inherently order-*dependent* — it
requires replaying a workername's own shares in true chronological
(`createdate`) order to correctly detect a boundary. `worker_sessions.py`
therefore gathers every new share tuple across every sharelog file
touched in a run into one batch, sorts it per workername, and replays
it against a single persistent global per-`(workername, enonce1)` state
dict, rather than combining independent per-file partials afterward.
First-run full-history replay (correctly reconstructing every
worker's true current session from this pool's entire history, not an
approximation) falls out of this design for free: with no state file
yet, every sharelog file's byte offset starts at 0, exactly like
`histogram_builder.py`'s own first-run "Total" dataset behavior — the
same "read sharelog bytes an extra time in exchange for genuine module
isolation" tradeoff already established for that module, not a new
one-off migration. A worker who has never submitted a share with a
valid workername/`enonce1`/`createdate` still gets a well-formed
(zero-filled, `session_started_at` null) result, never a missing key.
